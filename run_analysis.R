...
#making template vacant dataframe to save the data
m <- data.frame(matrix(nrow=length(unique(data5[,1]))*length(unique(data5[,2])),ncol = 68))

#labering column names 
colnames(m)<-colnames(data5)

#making combinations(n=30*6=180) of each subject(n=30) and each activity(n=6)
#l is a counter
...
#appending average data for for each combination to make final dataframe called "m"
#p is a counter
